...
class CVFFNN(rpnn.NeuralNetwork): 
    ...
    def backprop(self, y, y_pred, epoch):
        ...
        for layer in reversed(self.layers):
            ...
            if last==True:
                deltaDir = np.multiply(np.conj(deriv),e) 
                
                last = False
                
            else:
                deltaDir = np.multiply(np.conj(deriv),np.dot(np.conj(auxW.T),deltaDir))
             
            
            auxW = layer.weights
              
            layer._dweights = np.dot(deltaDir,np.conj(layer.input)) - (regFunc.l2_regularization(layer.lambda_init, layer.reg_strength, epoch)if layer.reg_strength else 0)*layer.weights
            ...
            layer._dbiases =  deltaDir.T - (regFunc.l2_regularization(layer.lambda_init, layer.reg_strength, epoch)if layer.reg_strength else 0)*layer.biases
